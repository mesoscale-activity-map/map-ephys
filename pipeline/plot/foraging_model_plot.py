...
def plot_session_fitted_choice(sess_key={'subject_id': 473361, 'session': 47},
    ...
    # Fetch actual data
    choice_history, reward_history, p_reward, _ = foraging_model.get_session_history(sess_key, remove_ignored=remove_ignored)
    ...
    ax = plot_session_lightweight([choice_history, reward_history, p_reward],
                                  smooth_factor=smooth_factor, ax=ax)

    # -- Plot fitted choice probability etc. --
    if model_id is not None:
        to_plot = results[results.model_id == model_id]
    else:
        to_plot = pd.concat([results.iloc[:first_n], results.iloc[len(results) - last_n:]])

    for idx, result in to_plot.iterrows():
