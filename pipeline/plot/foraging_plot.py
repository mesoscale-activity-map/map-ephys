...
# %%
def merge_dataframes_with_nans(df_1, df_2, basiscol='trial'):
    colstoadd = list()
    # =============================================================================
...
def extract_trials(plottype='2lickport',
    ...
    df_behaviortrial = pd.DataFrame(np.asarray(behaviortrial).T, columns=trial_attrs)
    ...
    for session in unique_sessions:
        total_trials_so_far = (
                foraging_analysis.SessionStats() & 'subject_id = {}'.format(subject_id) & 'session < {}'.format(
                session)).fetch('session_total_trial_num')
        bias_check_trials_now = (
                foraging_analysis.SessionStats() & 'subject_id = {}'.format(subject_id) & 'session = {}'.format(
                session)).fetch1('session_bias_check_trial_num')
        ...
        df_behaviortrial.loc[df_behaviortrial['session'] == session, 'ignore_rate'] = np.convolve(
            df_behaviortrial.loc[df_behaviortrial['session'] == session, 'outcome'] == 'ignore', kernel, 'same')
        reaction_time_interpolated = np.asarray(pd.DataFrame(
            np.asarray(df_behaviortrial.loc[df_behaviortrial['session'] == session, 'reaction_time'].values,
                       float)).interpolate().values.ravel().tolist()) * 1000
        df_behaviortrial.loc[df_behaviortrial['session'] == session, 'reaction_time_smoothed'] = np.convolve(
            reaction_time_interpolated, kernel, 'same')
        df_behaviortrial.loc[df_behaviortrial['session'] == session, 'trial'] += total_trials_so_far

        if type(filters) == dict:
            max_idx = (df_behaviortrial.loc[df_behaviortrial['session'] == session, 'ignore_rate'] > filters[
                'ignore_rate_max'] / 100).idxmax()

            session_first_trial_idx = (df_behaviortrial['session'] == session).idxmax()
            # print(max_idx)
            if max_idx > session_first_trial_idx or df_behaviortrial['ignore_rate'][session_first_trial_idx] > filters[
                'ignore_rate_max'] / 100:
                df_behaviortrial.loc[df_behaviortrial.index.isin(np.arange(max_idx, len(df_behaviortrial))) & (
                            df_behaviortrial['session'] == session), 'keep_trial'] = 0
    ...
    if not show_bias_check_trials:
        realtraining = (df_behaviortrial['p_reward_left'] < 1) & (df_behaviortrial['p_reward_right'] < 1) & (
                    (df_behaviortrial['p_reward_middle'] < 1) | df_behaviortrial['p_reward_middle'].isnull())
    ...
    reward_rate_right = np.asarray(
        (df_behaviortrial['trial_choice'] == 'right') & (df_behaviortrial['outcome'] == 'hit'))
    reward_rate_left = np.asarray((df_behaviortrial['trial_choice'] == 'left') & (df_behaviortrial['outcome'] == 'hit'))
    reward_rate_middle = np.asarray(
        (df_behaviortrial['trial_choice'] == 'middle') & (df_behaviortrial['outcome'] == 'hit'))
    ...
    # =============================================================================
    choice_rate_right = np.convolve(choice_right, kernel, 'same') / np.convolve(choice_left + choice_middle, kernel,
                                                                                'same')
    reward_rate_right = np.convolve(reward_rate_right, kernel, 'same') / np.convolve(
    ...
    for center_trial in np.arange(np.round(fit_window / 2), len(df_behaviortrial), fit_step):
        ...
        reward_rates_now = reward_rate_right[
                           int(np.round(center_trial - fit_window / 2)):int(np.round(center_trial + fit_window / 2))]
...
def plot_rt_iti(df_behaviortrial,
    ...
    if len(blockswitches) > 0:
        for trialnum_now in blockswitches:
            ax1.plot([df_behaviortrial['trial'][trialnum_now], df_behaviortrial['trial'][trialnum_now]], [0, 1000],
                     'b--')
            ax2.plot([df_behaviortrial['trial'][trialnum_now], df_behaviortrial['trial'][trialnum_now]], [0, 1000],
                     'b--')

    delay_smoothed = np.asarray(
        pd.DataFrame(np.asarray(df_behaviortrial['delay'].values, float)).interpolate().values.ravel().tolist())
    delay_smoothed = np.convolve(delay_smoothed, kernel, 'same')
    ax1.plot(df_behaviortrial['trial'], df_behaviortrial['reaction_time_smoothed'],
             'k-')  # np.convolve(RT,kernel,'same')
    ...
    ax11 = ax1.twinx()
    ax11.plot(df_behaviortrial['trial'], df_behaviortrial['ignore_rate'],
              'g-')  # np.convolve(df_behaviortrial['outcome']=='ignore',kernel,'same')
    ax11.set_ylim([0, 1])
...
def plot_trials(df_behaviortrial,
    ...
    if plottype == '2lickport':
        ...
        df_behaviortrial['reward_ratio'] = np.asarray(df_behaviortrial['p_reward_right'], float) / np.asarray(
            df_behaviortrial['p_reward_right'] + df_behaviortrial['p_reward_left'], float)

        bias = np.convolve(trial_choice_plot_interpolated, choice_filter, mode = 'valid')
        bias = np.concatenate((np.nan * np.ones(int(np.floor((len(choice_filter) - 1) / 2))), bias,
                               np.nan * np.ones(int(np.ceil((len(choice_filter) - 1) / 2)))))
    elif plottype == '3lickport':
        df_behaviortrial['reward_ratio_1'] = df_behaviortrial['p_reward_left'] / (
                    df_behaviortrial['p_reward_right'] + df_behaviortrial['p_reward_left'] + df_behaviortrial[
                'p_reward_middle'])
        df_behaviortrial['reward_ratio_2'] = (df_behaviortrial['p_reward_left'] + df_behaviortrial[
            'p_reward_middle']) / (df_behaviortrial['p_reward_right'] + df_behaviortrial['p_reward_left'] +
                                   df_behaviortrial['p_reward_middle'])
        # %
        leftchoices_filtered = np.convolve(df_behaviortrial['trial_choice'] == 'left', choice_filter, mode = 'valid')
        leftchoices_filtered = np.concatenate((np.nan * np.ones(int(np.floor((len(choice_filter) - 1) / 2))),
                                               leftchoices_filtered,
                                               np.nan * np.ones(int(np.ceil((len(choice_filter) - 1) / 2)))))
        rightchoices_filtered = np.convolve(df_behaviortrial['trial_choice'] == 'right', choice_filter, mode = 'valid')
        rightchoices_filtered = np.concatenate((np.nan * np.ones(int(np.floor((len(choice_filter) - 1) / 2))),
                                                rightchoices_filtered,
                                                np.nan * np.ones(int(np.ceil((len(choice_filter) - 1) / 2)))))
        middlechoices_filtered = np.convolve(df_behaviortrial['trial_choice'] == 'middle', choice_filter,
                                             mode = 'valid')
        middlechoices_filtered = np.concatenate((np.nan * np.ones(int(np.floor((len(choice_filter) - 1) / 2))),
                                                 middlechoices_filtered,
                                                 np.nan * np.ones(int(np.ceil((len(choice_filter) - 1) / 2)))))
        allchoices_filtered = np.convolve(df_behaviortrial['trial_choice'] != 'none', choice_filter, mode = 'valid')
        allchoices_filtered = np.concatenate((np.nan * np.ones(int(np.floor((len(choice_filter) - 1) / 2))),
                                              allchoices_filtered,
                                              np.nan * np.ones(int(np.ceil((len(choice_filter) - 1) / 2)))))
    ...
    if len(blockswitches) > 0:
        for trialnum_now in blockswitches:
            ax1.plot([df_behaviortrial['trial'][trialnum_now], df_behaviortrial['trial'][trialnum_now]], [-.15, 1.15],
                     'b--')

    if plottype == '2lickport':
        if plot_every_choice:
            ax1.plot(df_behaviortrial['trial'][rewarded], df_behaviortrial['trial_choice_plot'][rewarded], 'k|',
                     color = 'black', markersize = 30, markeredgewidth = 2)
            ax1.plot(df_behaviortrial['trial'][unrewarded], df_behaviortrial['trial_choice_plot'][unrewarded], '|',
                     color = 'gray', markersize = 15, markeredgewidth = 2)
        ...
        ax1.stackplot(np.asarray(df_behaviortrial['trial'], float), leftchoices_filtered / allchoices_filtered,
                      middlechoices_filtered / allchoices_filtered, rightchoices_filtered / allchoices_filtered,
                      colors = ['r', 'g', 'b'], alpha = 0.3)
        if plot_every_choice:
            ax1.plot(df_behaviortrial['trial'][rewarded], df_behaviortrial['trial_choice_plot'][rewarded], 'k|',
                     color = 'black', markersize = 30, markeredgewidth = 2)
            ax1.plot(df_behaviortrial['trial'][unrewarded], df_behaviortrial['trial_choice_plot'][unrewarded], '|',
...
def plot_efficiency_matching_bias(ax3,
    ...
    df_blockefficiency = pd.DataFrame(
        foraging_analysis.BlockEfficiency() * foraging_analysis.BlockStats() * foraging_analysis.SessionTaskProtocol() &
        'subject_id = {}'.format(subject_id) &
        'session >= {}'.format(sessions[0]) &
        'session <= {}'.format(sessions[1]) &
    ...
    for session in unique_sessions:
        ...
        total_trials_now = (
                foraging_analysis.SessionStats() & 'subject_id = {}'.format(subject_id) & 'session = {}'.format(
                session)).fetch1('session_total_trial_num')
        session_end_trial_nums.append(total_trials_so_far + total_trials_now)
        # bias_check_trials_now = (behavior_foraging.SessionStats()&'subject_id = {}'.format(subject_id) &'session = {}'.format(session)).fetch1('session_bias_check_trial_num')

        df_blockefficiency.loc[
            df_blockefficiency['session'] == session, 'session_start_trialnum'] += total_trials_so_far
        ...
        for block in blocks:
            ...
            df_blockefficiency.loc[
                block_idx_now, 'trialnum_block_middle'] = total_trials_so_far + trial_num_so_far + blocktrialnum / 2
    ...
    for session_now in sessions:
        ...
        if len(sessionidx) > 0:
            session_middle_trial_nums.extend(
                (np.asarray(session_start_trial_nums)[sessionidx] + np.asarray(session_end_trial_nums)[sessionidx]) / 2)
        else:
...
def plot_local_efficiency_matching_bias(df_behaviortrial, ax3):
    ...
    if len(blockswitches) > 0:
        for trialnum_now in blockswitches:
            ax3.plot([df_behaviortrial['trial'][trialnum_now], df_behaviortrial['trial'][trialnum_now]], [-.15, 1.15],
                     'b--')

    # %
    trialnums = df_behaviortrial.loc[~np.isnan(df_behaviortrial['local_matching_slope']), 'trial']
    local_matching_slope = df_behaviortrial.loc[
        ~np.isnan(df_behaviortrial['local_matching_slope']), 'local_matching_slope']
    local_matching_bias = df_behaviortrial.loc[
        ~np.isnan(df_behaviortrial['local_matching_slope']), 'local_matching_bias']
