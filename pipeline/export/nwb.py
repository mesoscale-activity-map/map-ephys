...
from nwb_conversion_tools.datainterfaces.behavior.movie.moviedatainterface import (
    MovieInterface,
)
...
warnings.filterwarnings("default", message=".*not found")
...
def datajoint_to_nwb(session_key, raw_ephys=False, raw_video=False):
    ...
    nwbfile.subject = pynwb.file.Subject(
        ...
        date_of_birth=datetime.combine(subject["date_of_birth"], zero_time)
    ...
    if dj.__version__ >= "0.13.0":
        units_query = (
            (ephys.ProbeInsertion.RecordingSystemSetup * ephys.Unit & session_key)
            .proj("unit_amp", "unit_snr", "electrode", "waveform")
            .join(ephys.UnitStat, left=True)
            .join(ephys.MAPClusterMetric.DriftMetric, left=True)
            .join(ephys.ClusterMetric, left=True)
            .join(ephys.WaveformMetric, left=True)
        )
    else:
        units_query = (
            (ephys.ProbeInsertion.RecordingSystemSetup * ephys.Unit & session_key)
            ...
            .aggr(
                ...
                **{
                    n: n
                    for n in ephys.MAPClusterMetric.DriftMetric.heading.names
                    if n not in ephys.MAPClusterMetric.DriftMetric.heading.primary_key
                },
                keep_all_rows=True,
            )
            .aggr(
    ...
    for insert_key in (ephys.ProbeInsertion & session_key).fetch("KEY"):
        ...
        if ephys.ProbeInsertion.InsertionLocation & insert_key:
            insert_location = {
                k: str(v)
                for k, v in (ephys.ProbeInsertion.InsertionLocation & insert_key)
                .aggr(
        ...
        electrode_config = (
            lab.Probe * lab.ProbeType * lab.ElectrodeConfig * ephys.ProbeInsertion
            & insert_key
        ).fetch1()
        ephys_device_name = (
            f'{electrode_config["probe"]} ({electrode_config["probe_type"]})'
        )
        ephys_device = (
            nwbfile.get_device(ephys_device_name)
        ...
        for electrode in electrode_query.fetch(as_dict=True):
            nwbfile.add_electrode(
                id=electrode["electrode"],
                group=electrode_group,
                filtering="",
                imp=-1.0,
                **electrode_ccf.get(
                    electrode["electrode"], {"x": np.nan, "y": np.nan, "z": np.nan}
                ),
        ...
        electrode_ind = electrode_df.index[
            electrode_df.group_name == electrode_group.name
        ]
            ...
            for feature, feature_tbl in tracking.Tracking().tracking_features.items():
                ...
                if feature_tbl & trk_device & session_key:
                    ...
                    for r in additional_conditions:
                        ...
                        tracking_timestamps = np.hstack(
                            [
                                np.arange(nsample) / trk_fs + float(trial_start_time)
                                for nsample, trial_start_time in zip(
                                    samples, start_time
                                )
                            ]
                        )
    ...
    for trial_event_type in (experiment.TrialEventType & q_trial_event).fetch(
        "trial_event_type"
    ):
        trial, event_starts, event_stops = (
            q_trial_event & {"trial_event_type": trial_event_type}
        ).fetch("trial", "event_start", "event_stop", order_by="trial")
    ...
    if raw_video:
        tracking_files_info = (
            tracking_ingest.TrackingIngest.TrackingFile & session_key
        ).fetch(as_dict=True, order_by="tracking_device, trial")
